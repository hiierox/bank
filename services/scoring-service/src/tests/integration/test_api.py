...
from app.api.scoring.schemas import AntifraudCheckResponse
from app.dependencies import (
    get_antifraud_http_client,
    get_http_client,
...
MOCK_HTTP_CLIENT_ANTIFRAUD = AsyncMock(spec=httpx.AsyncClient)
MOCK_PASSED_ANTIFRAUD_RESPONSE = MagicMock(spec=httpx.Response, status_code=200)
MOCK_PASSED_ANTIFRAUD_RESPONSE.json.return_value = AntifraudCheckResponse(decision='passed', reasons=[]).model_dump()
...
def client_with_mocked_deps():
    ...
    app.dependency_overrides[get_antifraud_http_client] = lambda: MOCK_HTTP_CLIENT_ANTIFRAUD

    MOCK_HTTP_CLIENT_ANTIFRAUD.post.return_value = MOCK_PASSED_ANTIFRAUD_RESPONSE

    test_client = TestClient(app)
    yield test_client, mock_http_client, mock_kafka_producer, MOCK_HTTP_CLIENT_ANTIFRAUD

    del app.dependency_overrides[get_http_client]
    del app.dependency_overrides[get_kafka_producer_service]
    del app.dependency_overrides[get_antifraud_http_client]
    MOCK_HTTP_CLIENT_ANTIFRAUD.reset_mock()


def test_pioneer_accepted_api(client_with_mocked_deps):
    """Pioneer успех, 'accepted', отправка в Kafka."""
    test_client, _, mock_kafka, mock_antifraud_http = client_with_mocked_deps
    request_data = {
    ...
    mock_antifraud_http.post.assert_called_once()
...
def test_pioneer_validation_error_api(client_with_mocked_deps):
    ...
    test_client, _, mock_kafka, _ = client_with_mocked_deps
...
def test_repeater_accepted_api(mock_get_status, client_with_mocked_deps):
    ...
    test_client, mock_http, mock_kafka, mock_antifraud_http = client_with_mocked_deps
...
def test_repeater_user_not_found_api(client_with_mocked_deps):
    ...
    test_client, mock_http, mock_kafka, _ = client_with_mocked_deps
