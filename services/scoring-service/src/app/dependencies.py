...
def get_antifraud_http_client(request: Request) -> httpx.AsyncClient:
    return cast(httpx.AsyncClient, request.app.state.http_client_antifraud)


def get_antifraud_service(
    client: httpx.AsyncClient = Depends(get_antifraud_http_client),
) -> AntifraudService:
    return AntifraudService(client=client)
