...
MOCK_AVAILABLE_PRODUCTS_PIONEER = [
    {'name': 'Кредит Базовый', 'amount': 'от 10 000 до 20 000', 'percentage': '15%'}
]
MOCK_AVAILABLE_PRODUCTS_REPEATER = [
    {'name': 'LoyaltyLoan', 'amount': '5000000', 'percentage': '1.8'}
]
...
async def test_flow_repeater(flow_service_fixture):
    ...
    product_repo.get_repeater_products.return_value = MOCK_AVAILABLE_PRODUCTS_REPEATER

    result = await flow_service.flow_type_selection(phone_number)

    assert result['flow_type'] == 'repeater'
    assert result['available_products'] == MOCK_AVAILABLE_PRODUCTS_REPEATER
...
async def test_flow_pioneer(flow_service_fixture):
    ...
    product_repo.get_pioneer_products.return_value = MOCK_AVAILABLE_PRODUCTS_PIONEER

    result = await flow_service.flow_type_selection(phone_number)

    assert result['flow_type'] == 'pioneer'
    assert result['available_products'] == MOCK_AVAILABLE_PRODUCTS_PIONEER
